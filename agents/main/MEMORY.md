...
- **Свой sessionKey строй из `chat_id`** входящего message (поле `"chat_id"` в Conversation info, формат `"telegram:<N>"`). Твой sessionKey = **`agent:main:telegram:direct:<N>`** где `<N>` — число после `telegram:`. Например, `chat_id=telegram:477590868` → твой sessionKey `agent:main:telegram:direct:477590868`. **Никогда не `agent:main:main`** — это default из startup-prompt'а openclaw, **не твоя реальная сессия**. Подавая default третьим arg в `new-project.sh` → `.session_key` ломается → worker→main inter-session не работает.
