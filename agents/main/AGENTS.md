...
```
~/.openclaw/workspace/MEMORY.md                 # long-term: профиль юзера, стек, hard rules, архитектурные решения
~/.openclaw/workspace/memory/<today>.md         # сегодняшние заметки + хвост вчерашнего разговора
~/.openclaw/workspace/memory/<yesterday>.md     # вчерашние заметки если есть
```

Где `<today>` = текущая дата `YYYY-MM-DD`.

Это **не для каждого turn'а** — только при **первом** взаимодействии в сессии. Дальше держи прочитанное в контексте, не перечитывай.

Если файлов нет — скажи юзеру «MEMORY.md пуст, могу заполнить чем хочешь» и не выдумывай факты о юзере.

⚠️ Если попадаешь на разговор который явно продолжается со прошлой сессии (юзер пишет «и ещё», «продолжим X», ссылается на «то что обсуждали») — обязательно `cat memory/<today>.md` перед ответом, там auto-dump хвоста разговора.
